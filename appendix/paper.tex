...
\texttt{AE\_FAF\_TIME} scales the time spent on fetch anchored fuzzing per snapshot.
...
For E1, you may notice there are 27 harnesses even though we cover 30 TAs. The reason is that the 3 Kinibi TAs also exist for Beanpod. 
TÄMU does not support Kinibi TAs but we can still cover Kinibi by simply reusing the Beanpod TÄMU harnesses. (The static analysis of 
copying shared memory was done on the actual Kinibi Tas).

The reproduction for C4 includes only a subset of the testing devices used for the experiment in the paper. 
