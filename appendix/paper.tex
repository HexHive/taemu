...
The artifact contains all the code and binaries to reproduce the paper's claim. 
...
\textbf{TODO: remote access over adb}

\subsection{Experiment Workflow}

All experiemnts excpet E10 are run from the \texttt{ae/ae.sh} scripts. 
...
\begin{itemize}
    \item (C1): ScHMuzz automatically detects crashes only triggerable via double fetches in TAs (Table 1, Figure 4, Figure 5). This is proven by experiments E1
    ...
    \item (C4): Reshaping-based approaches are less effective in targeting double fetches (Table 5). This is proven by experiment E4.
    \item (C5): Teegris, Mitee, QSEE, and Kinibi support zero-copy shared memory (Table 3). This is proven by experiment E5.
\end{itemize}
...
[Reshaping] [\textless1 compute-hour + 5 human-minutes]: Compares executions that triggered double fetches vs executions that did not.
...
\textit{[Execution]}
\small
\texttt{./ae/ae.sh e4\_reshaping}
\normalsize

\textit{[Results]}
The script prints the statistics in the same format as Table 5. The percentage of executions that triggered a double fetch should be similar to what is reported in the paper.
