...
channel defined by the GlobalPlatform API allows a app to call a command handling 
...
\begin{figure}[h]
  ...
  \caption{
    Shared memory facilitates communication between a app and a TA.
...
that are reachable by a app. Furthermore, static analysis for closed-source targets is more 
...
<<<<<<< HEAD
In \stagethree stage, we model each execution as a two-fetch trace \(v_x,v_y\), 
...
However, applying \sysname to our dataset in \autoref{sec:ecostudy} we did not encounter such a false positive.
...
After \stageone, we achieve between 15 - 30\% coverage of basic blocks reachable by a app.
...
if a memory region is shared with a app. By \emph{not} setting the \texttt{TEE\_MEMORY\_ACCESS\_ANY\_OWNER} 
