...
#define KG_CMD_UNLOCK   0x102u   /* PROBE (Agent2 repro-readiness): cmd 0x102 kg_verify_registration_info, the P2 length-math candidate. Was 0x10A. */
...
int main(void)
{
    ...
    /* PROBE: populate the per-cmd payload so kg_verify_registration_info's
     * length-parse / per-blob size-accumulation path (knxgud.md "kg_verify_
     * registration_info @0x18444", the longest handler) actually executes. Fill
     * the request tail with 0x41 and set a couple of plausible oversized 32-bit
     * length fields. We are measuring REACH DEPTH — which printf / which import
     * the handler halts on — NOT triggering a specific OOB. */
    memset(in + 4, 0x41, KG_MEMREF_SIZE - 4);
    *(uint32_t*)(in + OFF_CMD_ID) = KG_CMD_UNLOCK;        /* cmd_id at +0 */
    *(uint32_t*)(in + 4)  = 0xFFFFFFF0u;                  /* speculative len field #1 */
    *(uint32_t*)(in + 8)  = 0xFFFFFFF0u;                  /* speculative len field #2 */
