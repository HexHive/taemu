...
void load_functions()
{
    ...
    void *handle;
    char *error;
    /* OP-TEE normal-world client library (libteec). */
    handle = dlopen("libteec.so.1", RTLD_LAZY);
    if (!handle)
        handle = dlopen("libteec.so", RTLD_LAZY);
    if (!handle) {
        fprintf(stderr, "Failed to dlopen libteec: %s\n", dlerror());
        exit(EXIT_FAILURE);
    }
...
/* OP-TEE uses the canonical dashed UUID string (8-4-4-4-12); the field byte
 * order is identical to the teegris parser above. */
TEEC_UUID* optee_uuid(const char* ta){
    return teegris_uuid(ta);
}
