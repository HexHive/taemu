...
def qsee_read_relocs(ta_path:Path):
    ...
    for l in plt_lines:
        ...
        name = mtch.group(4)
        symbol_value = int(mtch.group(3), 16)
        if symbol_value != 0:
            print(f"qsee_read_relocs symbol-value non-zero: {name} {symbol_value:#0x}")
            continue
        out.append((name, offset_value))
    print(f"qsee_read_relocs: {out}")
    return out

def qsee_read_relocs_nonzero(ta_path:Path):
    raw = subprocess.check_output(
        f"readelf -r --use-dynamic --wide {ta_path}", shell=True
    ).decode()
    plt_off = raw.find("'PLT' relocation section")
    ...
    for l in plt_lines:
        ...
        if symbol_value == 0:
            continue
        out.append((name, offset_value, symbol_value))
    print(f"qsee_read_relocs: {out}")
    return out
...
def mitee_read_relocs(ta_path:Path):
    ...
    for l in rela_lines:
        mtch = re.match(
            r"([0-9a-z]+) +([0-9a-z]+) +(R_AARCH64_GLOB_DAT|R_AARCH64_ABS64) +0000000000000000 +([a-zA-Z_]+) \+",
            l,
        )
        if not mtch:
            continue
...
def mitee_rela_relocs(ta_path:Path):
    ...
    plt_off = raw.find("'PLT' relocation section")
    relr_off = raw.find("'RELA' relocation section")
    relr = raw[relr_off:plt_off]
    relr_lines = relr.split("\n")
    relr_lines = relr_lines[2:]
    out = []
