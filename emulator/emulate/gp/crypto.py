...
def TEE_AllocateOperation(ql: Qiling, hook_data):
    ...
    params = ql.os.resolve_fcall_params(
        {"operation": POINTER, "algorithm": UINT, "mode": UINT, "maxKeySize": UINT}
    )
    ...
    try:
        ...
        if param_algorithm == TEE_ALG_SHA256 and param_mode == TEE_MODE_DIGEST:
            ...
            OPERATION_ID += 1
        elif param_algorithm == TEE_ALG_RSASSA_PKCS1_PSS_MGF1_SHA256 and (
            param_mode == TEE_MODE_SIGN or param_mode == TEE_MODE_VERIFY
        ):
            ql.log.info("\tTEE_ALG_RSASSA_PKCS1_PSS_MGF1_SHA256")
            op = RSASSA_PKCS1_PSS_MGF1_SHA256_Operation(
                OPERATION_ID, param_mode, param_maxKeySize, ql
            )
            id2opration[OPERATION_ID] = op
            ql.mem.write_ptr(param_operation, OPERATION_ID)
            OPERATION_ID += 1
            ...
            op = TEE_ALG_RSAES_PKCS1_OAEP_MGF1_SHA256_Operation(
                OPERATION_ID, param_mode, param_maxKeySize, ql
            )
            id2opration[OPERATION_ID] = op
            ql.mem.write_ptr(param_operation, OPERATION_ID)
            ...
            ql.log.info(
                f"\t mode {hex(param_mode)} or algo {hex(param_algorithm)} not valid"
            )
...
def TEE_DigestUpdate(ql: Qiling, hook_data):
    global OPERATION_ID, id2opration
    params = ql.os.resolve_fcall_params(
        {"operation": UINT, "chunk": POINTER, "chunkSize": UINT}
    )
    param_operation = params["operation"]
    ...
    try:
        if param_operation not in id2opration:
            ql.log.error(
                f"TEE_DigestUpdate: Operation {hex(param_operation)} not in {id2opration}"
            )
            ql.emu_stop()
...
def TEE_DigestDoFinal(ql: Qiling, hook_data):
    ...
    try:
        if param_operation not in id2opration:
            ql.log.error(
                f"TEE_DigestDoFinal: Operation {hex(param_operation)} not in {id2opration}"
            )
            ql.emu_stop()
...
def TEE_FreeOperation(ql: Qiling, hook_data):
    ...
    if param_operation not in id2opration:
        ql.log.error(
            f"TEE_FreeOperation: Operation {hex(param_operation)} not in {id2opration}"
        )
        ql.emu_stop()
...
def TEE_SetOperationKey(ql: Qiling, hook_data):
    ...
    try:
        if param_operation not in id2opration:
            ql.log.error(
                f"TEE_SetOperationKey: Operation {hex(param_operation)} not in {id2opration}"
            )
            ql.emu_stop()

        if param_key not in handle2obj:
            ql.log.error(
                f"TEE_SetOperationKey: called with {hex(param_key)} not in {handle2obj}"
            )
            ql.emu_stop()
        ...
        if type(op) == RSAES_PKCS1_V1_5_Operation:
            if op.initialized:
                ql.log.error(
                    f"TEE_SetOperationKey: op is not an un-initialized operation type of TEE_ALG_RSAES_PKCS1_V1_5"
                )
                ql.emu_stop()

            if type(key) != RSA_KEYPAIR_Obj or not key.initialized:
                ql.log.error(
                    f"TEE_SetOperationKey: key is not a initialized object type of TEE_TYPE_RSA_KEYPAIR"
                )
                ql.emu_stop()
            ...
            if op.initialized:
                ql.log.error(
                    f"TEE_SetOperationKey: op is not an un-initialized operation type {type(op)}"
                )
                ql.emu_stop()

            if type(key) != AES_Obj or not key.initialized:
                ql.log.error(
                    f"TEE_SetOperationKey: key is not a initialized object type of TEE_TYPE_AES"
                )
                ql.emu_stop()
            ...
            op.initialize(key.rsa_param, ql)
            ql.log.info(
                "\tALG_RSAES_PKCS1_OAEP_MGF1_SHA256 initialized with new keys, passing..."
            )
...
def TEE_AsymmetricDecrypt(ql: Qiling, hook_data):
    ...
    if param_operation not in id2opration:
        ql.log.error(
            f"TEE_AsymmetricDecrypt: Operation {hex(param_operation)} not in {id2opration}"
        )
        ql.emu_stop()
    op = id2opration[param_operation]
...
def TEE_CipherInit(ql: Qiling, hook_data):
    global OPERATION_ID, id2opration
    params = ql.os.resolve_fcall_params(
        {"operation": UINT, "IV": POINTER, "IVLen": UINT}
    )
    param_operation = params["operation"]
    param_iv = params["IV"]
    ...
    if param_operation not in id2opration:
        ql.log.error(
            f"TEE_CipherInit: Operation {hex(param_operation)} not in {id2opration}"
        )
        ql.emu_stop()
    ...
    try:
        if type(op) == AES_ECB_NOPAD_Operation or type(op) == AES_CBC_NOPAD_Operation:
            if op.initialized and not op.active:
                ...
                ql.log.error(
                    f"TEE_CipherInit: {type(op)} not initialized or already activated"
                )
                ql.emu_stop()
...
def TEE_CipherDoFinal(ql: Qiling, hook_data):
    ...
    if param_operation not in id2opration:
        ql.log.error(
            f"TEE_CipherDoFinal: Operation {hex(param_operation)} not in {id2opration}"
        )
        ql.emu_stop()
...
def TEE_MACInit(ql: Qiling, hook_data):
    global OPERATION_ID, id2opration
    params = ql.os.resolve_fcall_params(
        {"operation": UINT, "iv": POINTER, "ivLen": POINTER}
    )
    param_operation = params["operation"]
    iv = params["iv"]
    ivLen = params["ivLen"]
    ...
    try:
        if type(op) == TEE_ALG_HMAC_SHA256_Operation:
            if op.activated:
                ...
                ql.mem.write(
                    macLen, len(result).to_bytes(ql.arch.pointersize, "little")
                )
            else:
                ql.log.error(f"TEE_CipherDoFinal: {type(op)} not activated")
                ql.emu_stop()
