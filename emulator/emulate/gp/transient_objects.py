...
def TEE_AllocateTransientObject(ql: Qiling, hook_data):
    ...
    params = ql.os.resolve_fcall_params(
        {"objectType": UINT, "maxObjectSize": UINT, "object": POINTER}
    )
...
def TEE_GenerateKey(ql: Qiling, hook_data):
    ...
    func_name = hook_data.func_name
    params = ql.os.resolve_fcall_params(
        {"object": POINTER, "keySize": UINT, "params": POINTER, "paramCount": UINT}
    )
    para_object = params["object"]
...
def TEE_PopulateTransientObject(ql: Qiling, hook_data):
    ...
    func_name = hook_data.func_name
    params = ql.os.resolve_fcall_params(
        {"object": POINTER, "attrs": POINTER, "attrCount": UINT}
    )
    para_object = params["object"]
    ...
    if para_object not in handle2obj:
        ql.log.error(
            f"TEE_PopulateTransientObject: called with {hex(para_object)} not in {handle2obj}"
        )
        ql.emu_stop()
    obj = handle2obj[para_object]
    ...
    if obj.initialized:
        ql.log.error(
            f"TEE_PopulateTransientObject: object {hex(para_object)} already initialized!!"
        )
        ql.emu_stop()
...
def TEE_ResetTransientObject(ql: Qiling, hook_data):
    ...
    if para_object not in handle2obj:
        ql.log.error(
            f"TEE_ResetTransientObject: called with {hex(para_object)} not in {handle2obj}"
        )
        crash(ql, func_name)
        return
    obj = handle2obj[para_object]
...
def TEE_FreeTransientObject(ql: Qiling, hook_data):
    ...
    if para_object not in handle2obj:
        ql.log.error(
            f"TEE_FreeTransientObject: called with {hex(para_object)} not in {handle2obj}"
        )
        crash(ql, func_name)
        return
    obj = handle2obj[para_object]
...
def TEE_InitRefAttribute(ql: Qiling, hook_data):
    ...
    func_name = hook_data.func_name
    params = ql.os.resolve_fcall_params(
        {"attr": POINTER, "attributeID": UINT, "buffer": POINTER, "length": UINT}
    )
    ...
    if (para_attributeID >> 29) & 0x1 != 0:
        ql.log.error(
            f"TEE_InitRefAttribute: attributeID {hex(para_attributeID)} not consistent"
        )
        ql.emu_stop()
...
def TEE_InitValueAttribute(ql: Qiling, hook_data):
    ...
    func_name = hook_data.func_name
    params = ql.os.resolve_fcall_params(
        {"attr": POINTER, "attributeID": UINT, "a": POINTER, "b": UINT}
    )
    para_attr = params["attr"]
    para_attributeID = params["attributeID"]
    para_a = params["a"]
...
def TEE_CopyObjectAttributes1(ql: Qiling, hook_data):
    ...
    ql.log.debug(
        f"TEE_CopyObjectAttributes1: dest {hex(para_destObject)}, src {hex(para_srcObject)}, {handle2obj}"
    )

    if para_destObject not in handle2obj:
        ql.log.error(
            f"TEE_CopyObjectAttributes1: called with {hex(para_destObject)} not in {handle2obj}"
        )
        ql.emu_stop()
    ...
    if para_srcObject not in handle2obj:
        ql.log.error(
            f"TEE_CopyObjectAttributes1: called with {hex(para_srcObject)} not in {handle2obj}"
        )
        ql.emu_stop()
