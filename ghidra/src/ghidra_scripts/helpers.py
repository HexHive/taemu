...
def cfg(program: ProgramDB, func: Function, disasm: bool = False) -> GraphHelper:
    ...
    Returns:
        Type[GraphHelper]: a graphhelper to render the cfg of `func`.
    """
...
def pretty_pcodeop(op: PcodeOp) -> str:
...
def pcode_cfg(func: Function) -> GraphHelper:
    animator = GraphHelper()

    options: DecompileOptions = DecompileOptions()
    monitor: ConsoleTaskMonitor = ConsoleTaskMonitor()
    ifc: DecompInterface = DecompInterface()
    ...
    res: DecompileResults = ifc.decompileFunction(func, 60, monitor)
    high: HighFunction = res.getHighFunction()

    blocks: List[PcodeBlockBasic] = high.getBasicBlocks()
    for block in blocks:

        aux_in_node_id = f"{block.getIndex()}-in"
        aux_out_node_id = f"{block.getIndex()}-out"
        # add an auxiliary node representing the entry to the basic block
        animator.add_node(aux_in_node_id, label=aux_in_node_id)

        op_iterator: Iterator[PcodeOp] = block.getIterator()
        prev_op: PcodeOp = None

        while op_iterator.hasNext():
            op = op_iterator.next()
            animator.add_node(op.getSeqnum(), label=pretty_pcodeop(op))
            if prev_op:
                animator.add_edge(prev_op.getSeqnum(), op.getSeqnum())
            else:
                # connect the auxiliary in-node to the first bb insn
                animator.add_edge(aux_in_node_id, op.getSeqnum())
            prev_op = op

        # add an auxiliary node representing the exit of the basic block
        animator.add_node(aux_out_node_id, label=aux_out_node_id)

        # connect the last bb insn to the auxiliary out-node
        animator.add_edge(prev_op.getSeqnum(), aux_out_node_id)

        nout = block.getOutSize()
        for idx in range(nout):
            aux_in_child_node_id = f"{block.getOut(idx).getIndex()}-in"
            animator.add_edge(aux_out_node_id, aux_in_child_node_id)
