...
def do_work(harness_path, apis, api_order_name):
    ...
    for iteration in range(0, FUZZ_ITERATIONS):
        ...
        while len(implemented_apis) <= len(apis):
            ...
            proc = subprocess.run(
                f"docker exec -it emu ./replay_api.sh ../{harness_path} {tmp_path}",
                shell=True,
                capture_output=True,
            )
...
def main():
    ...
    all_api_order = (
        open(os.path.join(BASE, "eval", "bbs_out", f"all_order.txt")).read().split("\n")
    )
    for tee in TEES:
        tee_api_order = (
            open(os.path.join(BASE, "eval", "bbs_out", f"{tee}_order.txt"))
            .read()
            .split("\n")
        )
