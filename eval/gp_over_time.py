...
for i, year in enumerate(years):
    ...
    for v in vendors:
        ...
        if gpv:
            ax.bar(x[i], gpv, bottom=gp_cum, color=color, width=bar_width, label=v if i == 0 else "")
            gp_cum += gpv

        # Downward for non-GP
        if ngpv:
            ax.bar(x[i], -ngpv, bottom=-nongp_cum, color=color, width=bar_width, label=v if i == 0 else "")
            nongp_cum += ngpv
