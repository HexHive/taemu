...
def worker(harness_path):
    ...
    subprocess.run(f'docker exec -e AFL_DEBUG=1 -e TAEMU_CRASH_NOTIMPL=1 -it emu ./fuzz.sh ../{harness_path}', shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
