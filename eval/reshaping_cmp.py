...
def fuzz(i, job):
    ...
    proc = subprocess.run(f'docker exec {emu_name} pkill python3', shell=True)
    proc = subprocess.run(
...
if __name__ == "__main__":
    ...
    # Remove leftover *emulator* workers only. The previous version removed
    # every container on the host (docker rm -f $(docker ps -aq)), which also
    # killed unrelated containers - never acceptable, and outright destructive
    # when this script itself runs inside a container with the docker socket.
    stale = [n for n in taemu_env.running_containers() if n.startswith("emu_")]
    if stale:
        a = input(f"remove stale emulator containers {stale} (y|N)? ")
        if a != "y":
            exit(0)
        os.system("docker rm -f " + " ".join(stale))
