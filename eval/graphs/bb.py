...
def cfg_ta(ta_path):
    ...
    for ta_entry in ta_fw:
        ...
        print(
            ta_path,
            ta_entry,
            len(bb_data["0x" + int2hex(ta_entry_addr).lstrip("0")]["nodes"]),
        )
...
def generate_graph(cfg, todo=None):
    ...
    if "BB_USE_CACHE" in os.environ and os.path.exists(f"{todo}_order.txt"):
        ...
        for api_name in api_order:
            ...
            implemented_apis.append(max_api)
            reachable.append(reachable_nodes(cfg, implemented_apis))
            if max_api.api_type == "gp_api":
                all_gp_idx = i
            if max_api.api_type == "libc":
                all_libc_idx = i
            if max_api.api_type == "tee_std":
                all_tee_std_idx = i
            if max_api.api_type == "tee":
                all_tee_idx = i
            i += 1
    else:
        while 1:
...
def gen_plot(
    reachable, max_nodes, all_gp_idx, all_libc_idx, all_tee_std_idx, all_tee_idx
):
    plt.clf()
    ...
    if all_tee_std_idx != 0:
        plt.axvline(
            all_tee_std_idx, color="orange", linestyle="--", label="TEE std index"
        )
    if all_tee_idx != 0:
...
def print_info(
    cfg, reachable, max_nodes, all_gp_idx, all_libc_idx, all_tee_std_idx, all_tee_idx
):
    ...
    print(
        f"% reachable with gp, libc and tee-std",
        100 * std_reachable / reachable[-1],
        "%",
    )
...
def build_tee_cfg(tee_path, only_tee=True, specific_tas=None):
    ...
    if specific_tas is None:
        for ta in [
            ta for ta in os.listdir(os.path.join(tee_path, "tas")) if ta.endswith(".ta")
        ]:
            ta_path = os.path.join(tee_path, "tas", ta)
            if not os.path.exists(ta_path[:-3] + ".json"):
                continue
    ...
    for cfg in ta_cfgs:
        ...
        if len([a for a in ta_apis if a.api_type == "gp_api"]) > 0:
            nr_tas_gp_api += 1
        if len([a for a in ta_apis if a.api_type == "libc"]) > 0:
            nr_tas_libc += 1
        if len([a for a in ta_apis if a.api_type.startswith("tee")]) > 0:
            nr_tas_tee += 1
...
def analyze_tee(tee_path):
    ...
    (
        ...
        all_tee_idx,
        implemented_apis,
    ) = generate_graph(tee_cfg, todo=tee)
    plt = gen_plot(
        reachable, max_nodes, all_gp_idx, all_libc_idx, all_tee_std_idx, all_tee_idx
    )
...
def analyze_all():
    ...
    (
        ...
        all_tee_idx,
        implemented_apis,
    ) = generate_graph(all_cfg, todo="all")
    plt = gen_plot(
        reachable, max_nodes, all_gp_idx, all_libc_idx, all_tee_std_idx, all_tee_idx
    )
