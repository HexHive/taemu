...
def _pinned_campaign(root):
    """The paper's campaign set, pinned in data/campaign_harnesses.txt.

    This repository carries harnesses beyond the paper's dataset (later TEE
    work), so discovering every harnessed TA would silently widen Table I.
    The pinned list keeps the artifact on the paper's 27 harnesses; set
    AE_CAMPAIGN_DISCOVER=1 to fall back to discovery."""
    if os.environ.get("AE_CAMPAIGN_DISCOVER"):
        return None
    path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                        "data", "campaign_harnesses.txt")
    ...
    for line in open(path):
        ...
        if not line:
            continue
        h = os.path.join(root, line)
        if not os.path.exists(os.path.join(h, "harness.py")):
            warn(f"pinned campaign harness missing: {line}")
            continue
        out.append(h)
