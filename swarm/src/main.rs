...
struct Args {
    ...
    #[arg(
        short,
        long,
        help = "Fuzzing script to run (defaults to <TAEMU_ROOT>/emulator/fuzz.sh)"
    )]
    fuzz_script: Option<PathBuf>,
...
/// The fuzzing script to run: --fuzz-script if given, otherwise
/// <TAEMU_ROOT>/emulator/fuzz.sh.
fn fuzz_script_of(args: &Args) -> PathBuf {
    args.fuzz_script
        .clone()
        .unwrap_or_else(|| config::repo_path("emulator/fuzz.sh"))
}

fn generate_fuzz_jobs(args: &Args) -> Vec<job_generation::FuzzJob> {
    let old_root = config::repo_path("emulator");
    let new_root = PathBuf::from("/srv/emulator");
    let fuzz_script_path =
        job_generation::rebase_path(fuzz_script_of(args), &old_root, &new_root);
...
async fn create_container_pool(
    ...
    for i in 0..std::cmp::min(args.max_parallel, num_containers) {
        let mut ct = container::Emulator::new(config::image_name(), format!("swarm_emu_{}", i));
...
fn create_redis_container() {
    let status = process::Command::new("docker")
        .arg("compose")
        .arg("-f")
        .arg(config::repo_path("docker-compose.redis.yml"))
        .arg("up")
        .arg("-d")
...
"#
    ...
    if !fuzz_script_of(&args).exists() {
        error!(LOGGER, "Fuzz script does not exist"; "path" => fuzz_script_of(&args).display());
        return 1;
    }
