...
pub trait Management: Send + Sync + 'static {
    ...
    fn _host_conf(&self) -> Option<HostConfig> {
        Some(HostConfig {
            binds: Some(vec![
                format!("{}:/srv", crate::config::repo_root().display()),
...
mod tests {
    ...
    async fn test_execute_command() {
        let mut container = Emulator::new(crate::config::image_name(), "test_container".to_string());
    ...
    async fn test_create_and_destroy() {
        let mut container = Emulator::new(crate::config::image_name(), "test_container2".to_string());
        container.create(None).await.unwrap();
