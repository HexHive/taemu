...
echo "y" | cargo run -- -t .. -f /root/TA_GP_emulator/emulator/fuzz.sh -d $MINUTES_ORIGINAL_FUZZING -p optee > "org-fuzz-output$(date +day%d-%H_%M_%S).log" 2>&1

echo "2. Deduplicating original fuzzing and generating suspicious_inputs_replay..."
cd /root/TA_GP_emulator
echo -e "y\ny" | python3 eval/deduplicate.py --mode coverage --enable-del --num-replay-containers 35 --per-harness-limit 10000
