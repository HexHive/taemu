...
fuzzer: Exploration, Fetch-Anchored Fuzzing, Distillation), the TA corpus and
the campaign data of the ecosystem-wide study, the proof-of-concept apps for the
discovered vulnerabilities, and the opt-in shared-memory mitigation for OP-TEE.
...
```sh
cd ae
./ae.sh setup      # builds the docker images and runs a self test (~20 min)
...
See [`ae/README.md`](ae/README.md) for the artifact appendix: claims, per
experiment runtimes, scaling knobs, and what cannot be reproduced without the
test phones.

The rest of this file documents the emulator itself for interactive use.
